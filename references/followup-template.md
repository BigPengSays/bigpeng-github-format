...
- 输入来源：（URL / 路径 / 粘贴）
- 第一步 — 仓库画像：（类型 / 核心能力 / 目标读者 / 推断依据）
- 第一步 — 保留的原有章节：（有 README 时列出；无 README 写「经典全套新建」）
- 第一步 — 规范化节名：（如 边界→常见问题）
- 第一步 — 新增章节：（仅 beautify 新补的节，无则写「无」）
- 第一步 — 导航草案：（标签 → ## → href 摘要）
- 第二步 — 初版要点：（迁移/生成说明；emoji 选用：H1 与各 ##）
- 第三步 — 润色说明：（Why / FAQ / 节间衔接等改动摘要）
- 第三步 — 真人化：（删套话、FAQ 改问答等；无大改写则写「语气微调」）
- 导航语言：（中文 / English / …）
- 导航项：（列出 Hero 锚点/外链）
- 锚点验证：（全部通过 / 修正项说明）
- 写入路径：（若已写入文件）
...
- 「第三步 — 润色说明」：至少列出 Why / FAQ 是否润色；无改动写「各节原文已通顺，未大改」。
- 「第三步 — 真人化」：列出主要去套话或 FAQ 改写；无则写「语气微调」。
