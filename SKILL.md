...
description: >-
  将 GitHub 仓库 README 格式化为 book-to-skill 风格的居中 Hero、徽章导航与分层文档结构。
...
把任意仓库 README 整理成 [book-to-skill](https://github.com/virgiliojr94/book-to-skill) 式首页：**居中 Hero、徽章带、可点击的锚点导航、分层结构**。

**核心原则：美化 ≠ 重写。** 保留 H1 与正文；**Why 必选**；节名可对齐 Usage / Install / FAQ；Hero 导航在正文 `##` 定稿后生成，**每个锚点必须可跳转**。
...
1. **信息采集** — 先判断输入模式  
   - **有 README**：以原文为权威；盘点全部 `##`、H1、首段、安装命令、边界说明  
   - **无 README**：读仓库文件（`SKILL.md`、`LICENSE`、`examples/`、`package.json` 等）推断内容；见 [references/no-readme-scaffold.md](references/no-readme-scaffold.md)  
   - 共用：[references/preserve-content.md](references/preserve-content.md)、[references/intake.md](references/intake.md)

2. **版块规划**（按输入模式分支）  

   **有 README**  
   - **尽量沿用原有版块**；原文有的节不删，只改排版或节名  
   - **节名尽量规范化**：对齐 Why / Usage / Install / FAQ / Examples；项目独有节（如「爆款标题的 7 种公式」）**保留原名**  
...
1. **保留 H1 与正文**：禁止用 repo 名替换产品名。见 [references/preserve-content.md](references/preserve-content.md)。
